...
program testTioga
  ...
  !>
  !if (myid==0) then
  ! write(6,*) 'receptorInfo(1:3)=',receptorInfo(1:3)
  ! write(6,*) 'inode=',inode(1:receptorInfo(3))
  ! write(6,*) 'frac=',frac(1:receptorInfo(3))
  !endif

  istor='row'
  call tioga_dataupdate(g%q,g%nvar,istor)      !< update the q-variables (can be called anywhere)
  ...
  if (myid==0) write(6,*) '----------------------------'
  if (myid==0) write(6,*) '#procid  interpolation-error'
  if (myid==0) write(6,*) '----------------------------'

  write(6,"(I10,3x,E14.7)") myid,sqrt(rnorm/g%nv/g%nvar)
  call cpu_time(t2)
  
  call mpi_barrier(mpi_comm_world,ierr)
  if (myid==0) write(6,*) 'writing flow visualization files'   
  call tioga_writeoutputfiles(g%q,g%nvar,istor) !< write output files, if need be

200 continue    
  call mpi_barrier(mpi_comm_world,ierr)
  if (myid==0) write(6,*) 'finished writing files..'
  call tioga_delete
